...
@dataclass
class L241RealValidationArtifacts:
...
class L241RealValidationRunner:
    def __init__(
        self,
        *,
    ...
    def run(
        self,
        *,
        case_id: str = 'case_l241_real',
        query_text: str = 'procedimento responsabile motivazione accesso legge 241',
        trace_id: str = 'trace_l241_real',
        search_item_path: Optional[str | Path] = None,
        detail_payload_path: Optional[str | Path] = None,
        requested_output_type: str = 'RAC_DRAFT',
    ) -> L241RealValidationArtifacts:
        fixture_dir = self.default_fixture_dir()
        search_item_path = Path(search_item_path or (fixture_dir / 'l241_search_item_real.json'))
        detail_payload_path = Path(detail_payload_path or (fixture_dir / 'l241_detail_payload_real.json'))
        ...
        if ingest_result.source_document is None:
            raise RuntimeError('Ingestion L.241/1990 reale fallita: SourceDocument assente')
        ...
        retrieval_query, retrieval_results = self.retrieval_service.query(
            ...
            domain_target='procedimento_amministrativo',
            top_k=10,
            trace_id=trace_id,
        )
        ...
        return L241RealValidationArtifacts(
