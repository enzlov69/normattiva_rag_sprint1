...
def riaccertamento_top3_purity_ok(ranked: Sequence[Candidate]) -> bool:
    if len(ranked) < 3:
        return False
    profiles = [riaccertamento_anchor_profile(candidate_anchor_text(c, "riaccertamento_ordinario")) for c in ranked[:3]]
    if not profiles[0]["strong"] or not profiles[1]["process_anchor"]:
        return False
    return profiles[2]["process_anchor"]
...
def maybe_promote_riaccertamento_pure_top3(ranked: List[Candidate]) -> List[Candidate]:
    ...
    if top3_profiles[2]["process_anchor"]:
        return ranked
    ...
    for idx in range(3, len(ranked)):
        ...
        if not profile["process_anchor"]:
            continue
        neg_hits = candidate_negative_hits(ranked[idx], "riaccertamento_ordinario")
...
def print_candidate(idx: int, candidate: Candidate) -> None:
    ...
    if candidate.debug.get('riacc_hard_hits') or candidate.debug.get('riacc_soft_hits'):
        print(f"  riacc_hard_hits: {candidate.debug.get('riacc_hard_hits')} | riacc_soft_hits: {candidate.debug.get('riacc_soft_hits')} | process_anchor: {candidate.debug.get('riacc_process_anchor')} | strong_anchor: {candidate.debug.get('riacc_strong_anchor')}")
...
def print_final_command_hint() -> None:
    ...
    print(
        'py tests/run_router_federato_generale_base_v25_3.py --persist-dir data/chroma --query-text "fpv" --query-text "impegno di spesa" --query-text "esigibilità della spesa" --query-text "riaccertamento ordinario dei residui" --query-text "cronoprogramma investimento" --query-text "entrate vincolate"'
    )


def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Router federato generale v25.3 - focus DLgs. 118 sotto-intenti specialistici")
    ...
    print("=== ROUTER FEDERATO GENERALE — BASE v25.3 ===")
