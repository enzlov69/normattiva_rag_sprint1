...
def riaccertamento_anchor_profile(blob: str) -> Dict[str, Any]:
    hard_hits = [normalize_text(x) for x in RIACCERTAMENTO_HARD_ANCHORS if normalize_text(x) in blob]
    soft_hits = [normalize_text(x) for x in RIACCERTAMENTO_SOFT_ANCHORS if normalize_text(x) in blob]
    strong = (len(hard_hits) >= 1 and (len(hard_hits) + len(soft_hits)) >= 2) or len(hard_hits) >= 2
    return {
        "hard_hits": hard_hits,
        "soft_hits": soft_hits,
...
def candidate_has_strong_anchor(candidate: Candidate, subintent: str) -> bool:
    ...
    profile = riaccertamento_anchor_profile(candidate_blob(candidate))
...
def false_positive_penalty(candidate: Candidate, subintent: str, anchor_hits: int, rescue_mode: bool = False) -> float:
    ...
    if subintent == "riaccertamento_ordinario":
        profile = riaccertamento_anchor_profile(blob)
        hard_count = int(profile["hard_count"])
        strict_negative_hits = count_hits(blob, [normalize_text(x) for x in RIACCERTAMENTO_STRICT_NEGATIVES])

        if strict_negative_hits > 0 and hard_count == 0:
            penalty += strict_negative_hits * (34.0 if not rescue_mode else 52.0)
        elif strict_negative_hits > 0 and hard_count == 1:
            penalty += strict_negative_hits * (10.0 if not rescue_mode else 16.0)
...
def score_candidate(candidate: Candidate, query_text: str, subintent: str, rescue_mode: bool = False) -> float:
    ...
    riacc_profile = None
    if subintent == "riaccertamento_ordinario":
        riacc_profile = riaccertamento_anchor_profile(blob)
        if riacc_profile["hard_count"] >= 1:
            anchor_score += 24.0 * riacc_profile["hard_count"]
        if riacc_profile["strong"]:
            anchor_score += 42.0 if not rescue_mode else 65.0

    score = (sim_score + variant_bonus + anchor_score + hint_score - penalty_score) * collection_prior
    ...
    if rescue_mode and subintent == "riaccertamento_ordinario":
        ...
        if not strong_anchor:
            score *= 0.62
        if negative_hits > 0 and hard_count == 0:
            score *= 0.35
...
def should_trigger_rescue(ranked: Sequence[Candidate], subintent: str) -> bool:
    ...
    top3 = list(ranked[:3])
    top2 = list(ranked[:2])
    if not any(candidate_has_strong_anchor(c, subintent) for c in top3):
        return True

    top1 = top3[0]
    if not candidate_has_strong_anchor(top1, subintent):
        return True

    for cand in top2:
        neg_hits = candidate_negative_hits(cand, subintent)
        profile = riaccertamento_anchor_profile(candidate_blob(cand))
        if neg_hits > 0 and int(profile["hard_count"]) == 0:
            return True
...
def print_candidate(idx: int, candidate: Candidate) -> None:
    ...
    if candidate.debug.get('riacc_hard_hits') or candidate.debug.get('riacc_soft_hits'):
        print(f"  riacc_hard_hits: {candidate.debug.get('riacc_hard_hits')} | riacc_soft_hits: {candidate.debug.get('riacc_soft_hits')} | strong_anchor: {candidate.debug.get('riacc_strong_anchor')}")
    print(f"  hint_score: {candidate.debug.get('hint_score')} | penalty_score: {candidate.debug.get('penalty_score')}")
...
def print_final_command_hint() -> None:
    ...
    print(
        'py tests/run_router_federato_generale_base_v25_1.py --persist-dir data/chroma --query-text "fpv" --query-text "impegno di spesa" --query-text "esigibilità della spesa" --query-text "riaccertamento ordinario dei residui" --query-text "cronoprogramma investimento" --query-text "entrate vincolate"'
    )


def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Router federato generale v25.1 - focus DLgs. 118 sotto-intenti specialistici")
    ...
    print("=== ROUTER FEDERATO GENERALE — BASE v25.1 ===")
    ...
    for query_text in queries:
        ...
        if rescue_on:
            ranked = rank_candidates(list(all_candidates.values()), query_text, subintent, rescue_mode=True)
            anchor_hit = anchor_hit_in_top3(ranked, subintent)
