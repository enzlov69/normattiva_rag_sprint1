...
RIACCERTAMENTO_OPERATIONAL_PROCESS_TERMS = [
    "reimput",
    "cancellat",
]

RIACCERTAMENTO_STRONG_LATERAL_PATTERNS = [
    "rateizzazione",
    "entrate proprie",
    "accertamento dell'entrata",
    "riscossione",
...
def riaccertamento_operational_nucleus(anchor_text: str) -> bool:
    ...
    has_process_terms = contains_any(anchor_text, [normalize_text(x) for x in RIACCERTAMENTO_OPERATIONAL_PROCESS_TERMS])
    return has_residui and has_process_terms


def riaccertamento_core_combo(anchor_text: str, profile: Dict[str, Any]) -> bool:
    return bool(profile.get("process_anchor")) and riaccertamento_operational_nucleus(anchor_text)
...
def riaccertamento_lateral_for_top3(anchor_text: str, profile: Dict[str, Any]) -> bool:
    ...
    if lateral_hits == 0:
        return False
    strong_lateral = contains_any(anchor_text, [normalize_text(x) for x in RIACCERTAMENTO_STRONG_LATERAL_PATTERNS])
    if strong_lateral and not riaccertamento_operational_nucleus(anchor_text):
        return True
    return not riaccertamento_operational_nucleus(anchor_text)
...
def score_candidate(candidate: Candidate, query_text: str, subintent: str, rescue_mode: bool = False) -> float:
    ...
    if subintent == "riaccertamento_ordinario":
        ...
        operational_nucleus = riaccertamento_operational_nucleus(anchor_text)
        ...
        if process_anchor and not operational_nucleus:
            score *= 0.26 if not rescue_mode else 0.08
        elif process_anchor and not core_combo:
            score *= 0.70 if not rescue_mode else 0.34
        if lateral_top3:
            score *= 0.18 if not rescue_mode else 0.05
        ...
        if rescue_mode and negative_hits > 0 and not process_anchor:
            score *= 0.28
        if rescue_mode and process_anchor and not operational_nucleus:
            score *= 0.18
        elif rescue_mode and process_anchor and not core_combo:
            score *= 0.26
    ...
    candidate.debug.update(
        {
            ...
            "riacc_operational_nucleus": riaccertamento_operational_nucleus(anchor_text) if subintent == "riaccertamento_ordinario" else False,
...
def is_riaccertamento_top3_eligible(candidate: Candidate) -> bool:
    ...
    if not profile["process_anchor"]:
        return False
    if not riaccertamento_operational_nucleus(anchor_text):
        return False
...
def riaccertamento_top3_purity_ok(ranked: Sequence[Candidate]) -> bool:
    ...
    if not profiles[1]["process_anchor"] or not riaccertamento_core_combo(texts[1], profiles[1]):
        return False
    return (
        profiles[2]["process_anchor"]
        and riaccertamento_operational_nucleus(texts[2])
        and riaccertamento_core_combo(texts[2], profiles[2])
        and not riaccertamento_lateral_for_top3(texts[2], profiles[2])
    )
...
def print_candidate(idx: int, candidate: Candidate) -> None:
    ...
    if candidate.debug.get('riacc_hard_hits') or candidate.debug.get('riacc_soft_hits'):
        print(f"  riacc_hard_hits: {candidate.debug.get('riacc_hard_hits')} | riacc_soft_hits: {candidate.debug.get('riacc_soft_hits')} | process_anchor: {candidate.debug.get('riacc_process_anchor')} | strong_anchor: {candidate.debug.get('riacc_strong_anchor')} | core_combo: {candidate.debug.get('riacc_core_combo')} | operational_nucleus: {candidate.debug.get('riacc_operational_nucleus')} | lateral_top3: {candidate.debug.get('riacc_lateral_top3')}")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print("=== ROUTER FEDERATO GENERALE — BASE v25.6 ===")
