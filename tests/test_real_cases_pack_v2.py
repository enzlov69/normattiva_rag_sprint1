...
def build_case2_request() -> dict:
    return {
        ...
        "shadow": {"internal_use_only": True, "note": "Pack v2 - Caso reale 2"},
    }
...
def build_case3_request() -> dict:
    return {
        ...
        "audit": {"created_by": "LEVEL_A_RUNTIME", "internal_only": True},
        "shadow": {"internal_use_only": True, "note": "Pack v2 - Caso reale 3"},
    }
...
def test_real_cases_pack_v2_case1_degraded_valid() -> None:
...
def test_real_cases_pack_v2_case1_m07_blocked() -> None:
    request = build_case1_request()
...
def test_real_cases_pack_v2_case2_degraded_valid() -> None:
...
def test_real_cases_pack_v2_case2_m07_overreach_rejected() -> None:
    request = build_case2_request()
...
def test_real_cases_pack_v2_case3_degraded_valid() -> None:
    request = build_case3_request()

    def invoker(_: dict) -> dict:
        return build_case3_response_degraded()
...
def test_real_cases_pack_v2_case3_m07_blocked() -> None:
    request = build_case3_request()

    def invoker(_: dict) -> dict:
        return build_case3_response_blocked_m07()
...
def test_real_cases_pack_v2_case3_merit_overreach_rejected() -> None:
    request = build_case3_request()

    def invoker(_: dict) -> dict:
        return build_case3_response_invalid_merit_overreach()
...
def test_real_cases_pack_v2_all_requests_remain_non_decisional() -> None:
    for request in (build_case1_request(), build_case2_request(), build_case3_request()):
...
def test_real_cases_pack_v2_summary_state_is_coherent() -> None:
    scenarios = []

    scenarios.append(
        perform_runtime_roundtrip(
            build_case1_request(),
            lambda _: build_case1_response_degraded(),
        )
    )
    scenarios.append(
        perform_runtime_roundtrip(
            build_case2_request(),
            lambda _: build_case2_response_degraded(),
        )
    )
    scenarios.append(
        perform_runtime_roundtrip(
            build_case3_request(),
            lambda _: build_case3_response_degraded(),
        )
    )

    statuses = [scenario["runtime_status"] for scenario in scenarios]
    assert statuses == ["ROUNDTRIP_DEGRADED", "ROUNDTRIP_DEGRADED", "ROUNDTRIP_DEGRADED"]
