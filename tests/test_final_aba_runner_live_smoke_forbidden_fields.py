...
from runtime.final_aba_runner_real_invoker import (
    DEFAULT_FORBIDDEN_LEVEL_B_FIELDS,
    FederatedRunnerRealInvoker,
)
...
def _request() -> dict:
    return {
        "request_id": os.getenv("FEDERATED_RUNNER_LIVE_REQUEST_ID", "req_live_smoke_003"),
        "case_id": os.getenv("FEDERATED_RUNNER_LIVE_CASE_ID", "case_live_smoke_003"),
        "trace_id": os.getenv("FEDERATED_RUNNER_LIVE_TRACE_ID", "trace_live_smoke_003"),
        "api_version": "1.0",
...
def _scan_keys(data, hits: set[str]) -> None:
    if isinstance(data, dict):
        for key, value in data.items():
            if key in DEFAULT_FORBIDDEN_LEVEL_B_FIELDS:
                hits.add(key)
...
def test_live_runner_response_exposes_no_forbidden_level_b_fields():
    ...
    hits: set[str] = set()
    _scan_keys(result, hits)
    assert not hits, f"Forbidden Level B fields detected in live response: {sorted(hits)}"
    assert "m07_closed" not in hits
    assert "m07_completed" not in hits
    assert "m07_approved" not in hits
