...
def print_acceptance_notes(query_text: str, ranked: Sequence[Candidate]) -> None:
    ...
    if q == "impegno di spesa":
        ...
        has_anchor = any(int(c.debug.get("anchor_hits", 0)) >= 2 for c in ranked[:3])
...
def print_final_command_hint() -> None:
    ...
    print(
        'py tests/run_router_federato_generale_base_v25.py --persist-dir data/chroma --query-text "fpv" --query-text "impegno di spesa" --query-text "esigibilità della spesa" --query-text "riaccertamento ordinario dei residui" --query-text "cronoprogramma investimento" --query-text "entrate vincolate"'
    )


def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Router federato generale v25 - focus DLgs. 118 sotto-intenti specialistici")
    ...
    print("=== ROUTER FEDERATO GENERALE — BASE v25 ===")
    ...
    for query_text in queries:
        ...
        rescue_on = False
        if subintent in RESCUE_SUBINTENTS and not anchor_hit:
            rescue_on = True
